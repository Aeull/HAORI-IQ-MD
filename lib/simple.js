...
END:VCARD
 ...
             sections 
         } 
         conn.sendMessage(jid, showAdAttribution: true }, { qouted })
             quoted, 
             ephemeralExpiration: 86400, 
             contextInfo: { 
...
exports.smsg = (conn, m, hasParent) => {
     ...
     */
     m.reply = async (text, chatId, options) => {
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { title: 'ArullBotz「 MD 」☇', body: `❏ ${date}\n`, sourceUrl: 'https://bit.ly/3RqEgLG', thumbnail: fs.readFileSync('./thumbnail.jpg')}}, options })
    }
